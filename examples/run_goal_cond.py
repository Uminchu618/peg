...
def make_env(config, use_goal_idx=False, log_per_goal=False, eval=False):
    ...
    if "dmc" in config.task:
        ...
        env = envs.KitchenStatesEnv(
            action_repeat=config.action_repeat,
            use_goal_idx=use_goal_idx,
        ...
        from envs.sibrivalry.ant_maze import (
            AntMazeEnvFullDownscale,
            AntHardMazeEnvFullDownscale,
        )
        ...
        if config.task in {"fetchpnp", "fetchpnpeasy"}:
            from envs.customfetch.custom_fetch import (
                EasyPickPlaceEnv,
                PickPlaceEnv,
                GoalType,
            )
        ...
        from envs.customfetch.custom_fetch import (
            DemoStackEnv,
            WallsDemoStackEnv,
            DiscreteWallsDemoStackEnv,
        )

        if "walls" in config.task:
            if "disc" in config.task:
                env = DiscreteWallsDemoStackEnv(
                    max_step=config.time_limit, eval=eval, increment=0.01
                )
            else:
                n = int(config.task[-1])
                env = WallsDemoStackEnv(
                    max_step=config.time_limit, eval=eval, n=int(config.task[-1])
                )
        else:
        ...
        if n == 2:  # noisy dim
            ...
            obs_min = np.concatenate(
                [
                    env.workspace_min,
                    [0.0, 0.0],
                    *[env.workspace_min for _ in range(env.n)],
                    obs_min_noise,
                ],
                0,
            )
            ...
            obs_max = np.concatenate(
                [
                    env.workspace_max,
                    [0.05, 0.05],
                    *[env.workspace_max for _ in range(env.n)],
                    obs_max_noise,
                ],
                0,
            )
            ...
            def generate_img_from_state(states):
                ...
                for i, obs in enumerate(states):
                    ...
                    sim.data.set_joint_qpos(
                        "robot0:r_gripper_finger_joint", gripper_state[0]
                    )
                    sim.data.set_joint_qpos(
                        "robot0:l_gripper_finger_joint", gripper_state[1]
                    )
                    ...
                    env.sim.forward()
                    img = (
                        env.render("rgb_array", 200, 200).astype(np.float32) / 255.0
                    ) - 0.5
                    all_img.append(img)
...
def make_eval_fn(config):
    ...
    if (
        "dmc" in config.task
        or "mtmw" in config.task
        or "robobin" in config.task
        or "kitchen" in config.task
    ):
        if "dmc" in config.task:
        ...
        def episode_render_fn(env, ep):
            ...
            ep_img = np.pad(
                ep_img,
                ((0, (config.time_limit + 1) - T), (0, 0), (0, 0), (0, 0)),
                "constant",
                constant_values=(0),
            )
            executions.append(ep_img[None])  # 1 x T x H x W x C
        ...
        def evaluate_all_goals(driver, eval_policy, logger):
            ...
            logger.scalar(
                "max_eval_metric_success_cell/goal_all", all_metric_success_cell
            )
            logger.write()
        ...
        def evaluate_all_goals(driver, eval_policy, logger):
            env = driver._envs[0]
            num_goals = (
                len(env.get_goals()) if len(env.get_goals()) > 0 else 5
            )  # MEGA uses 30 episodes for eval.
            num_eval_eps = 10
            executions = []
            goals = []
            all_metric_success = []
        ...
        def episode_render_fn(env, ep):
            ...
            inner_env = env._env._env._env
            inner_env.g_xy = np.concatenate(
                (inner_env.goal_list[inner_env.goal_idx], other_dims)
            )
            a1_env.sim.forward()
            goal_img = env.render(mode="rgb_array")
            for obs in ep["observation"]:
        ...
        def episode_render_fn(env, ep):
            ...
            for obs in [ep["goal"][0], *ep["observation"]]:
                ...
                sim.forward()
                img = sim.render(
                    height=100, width=100, camera_name="external_camera_0"
                )[::-1]
                all_img.append(img)
            goals.append(all_img[0][None])  # 1 x H x W x C
            ep_img = np.stack(all_img[1:], 0)
    ...
    if "dmc_walker_walk_proprio" == config.task:
        ...
        def episode_render_fn(env, ep):
            ...
            for i, obs in enumerate(ep["observation"]):
                ...
                sim.forward()
                img = sim.render(
                    height=200, width=200, camera_name="external_camera_0"
                )[::-1]
    ...
    if "pointmaze" in config.task:
        ...
        def vis_fn(elite_inds, elite_samples, seq, wm, eval_env, logger):
            ...
            goal_states = tf.repeat(
                elite_samples[None, :num_vis], elite_obs.shape[0], axis=0
            ).numpy()  # T x topk x 2
        ...
        def vis_fn(elite_inds, elite_samples, seq, wm, eval_env, logger):
            ...
            p2evalue_ax.plot(traj[:, :, 0], traj[:, :, 1], c="b", zorder=4, marker=".")
    ...
    if "dmc_walker_walk_proprio" == config.task:
        ...
        def make_plot(
            ...
            for ep_count, episode in enumerate(episodes):
                ...
                if (
                    ep_count % batch_size == (batch_size - 1)
                    or ep_count == len(episodes) - 1
                ):
                    end = ep_count
            ...
            x_div = y_div = 100
            other_dims = np.array([0.34, 0.74, -1.34, -0.0, 1.1, -0.66, -0.1])
            gx = 5.0
            gy = 0.0
            ...
            obs = {
                "qpos": XY_plus,
                "goal": goal_vec,
                "reward": np.zeros(XY.shape[0]),
                "discount": np.ones(XY.shape[0]),
            ...
            if config.gc_reward == "dynamical_distance":
                ...
                dd_ax.scatter(
                    y=before[0][0], x=before[0][1], c="b", marker=".", s=20, zorder=2
                )
        ...
        def make_plot(
            ...
            for ep_count, episode in enumerate(episodes[::ep_subsample]):
                ...
                if (
                    ep_count % batch_size == (batch_size - 1)
                    or ep_count == len(episodes[::ep_subsample]) - 1
                ):
                    end = ep_count
                    # chunk = {k: tf.stack(v) for k, v in chunk.items()}
                    obs.append(tf.stack(chunk[obs_key]))
        ...
        def make_plot(
            ...
            def plot_axes(axes, data, cmap, title, zorder):
                for ax, pd in zip(axes, plot_dims):
                    ...
                    ax.set_title(
                        f"{title} {plot_dim_name[pd[0]]}{plot_dim_name[pd[1]]}",
                        fontdict={"fontsize": 10},
                    )

            fig, (g_ax, ob_ax, rew_ax, p2evalue_ax) = plt.subplots(
                1, 4, figsize=(13, 3)
            )
            ...
            obs = {
                ...
                "goal": goal_vec,
                "reward": np.zeros(len(XZ)),
                "discount": np.ones(len(XZ)),
                "is_terminal": np.zeros(len(XZ)),
            }
            temporal_dist = agnt.temporal_dist(obs)
            if config.gc_reward == "dynamical_distance":
                im = rew_ax.tricontourf(XZ[:, 0], XZ[:, 1], temporal_dist, zorder=1)
                rew_ax.scatter(
                    x=[goal_pos[0]], y=[goal_pos[2]], c="r", marker="*", s=20, zorder=2
                )
                rew_ax.scatter(
                    x=[start_pos[0]],
                    y=[start_pos[2]],
        ...
        def make_plot(
            ...
            for obj, axs in obj_to_ax.items():
                ...
                if obj == "kettle":  # only plot xy dims.
                    ...
                    "microwave",
                    "slide_cabinet",
                }:  # plot both 1D lines on same plot.
        ...
        def make_plot(
            ...
            for obj_ax, obj_pos, obj_g_pos, obj_color in zip(
                obj_axes, all_obj_obs_pos, all_obj_g_pos, obj_colors
            ):
            ...
            values = values.numpy().flatten()
            cm = plt.cm.get_cmap("viridis")
            for obj_ax, obj_pos, obj_color in zip(
                obj_axes, all_obj_obs_pos, obj_colors
            ):
                p2e_scatter = p2evalue_ax.scatter(
        ...
        "umazefulldownscale",
        "a1umazefulldownscale",
        "hardumazefulldownscale",
    }:
        ...
        def make_plot(
            ...
            if config.gc_reward == "dynamical_distance":
                ...
                dd_ax.scatter(
                    x=before[0][0], y=before[0][1], c="b", marker=".", s=20, zorder=2
                )
                plt.colorbar(td_plot, ax=dd_ax)
                dd_ax.set_title("temporal distance")
